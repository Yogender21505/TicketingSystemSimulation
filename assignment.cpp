...
// std::atomic<int> currentTicketNumber(-1); // Define and initialize the atomic variable
...
void executeTicketingSystemParticipation(int ticketNumber)
{
	...
	// Perform the thread task here

	// Log thread completion
	outputFile << "C++11: Thread with ticket number: " << ticketNumber << " completed.\n";
	outputFile.close();

	// Increment the current ticket number to allow the next thread to proceed
	currentTicketNumber++;
}
...
int runSimulation()
{
	...
	if (USERNAME == currentUser && USERNAME == userFromFile)
	{
		...
		currentTicketNumber.store(0); // Initialize the atomic variable

		std::thread threads[currentNumThreads];

		for (int threadIndex = 0; threadIndex < currentNumThreads; ++threadIndex)
		{
			int ticketNumber = threadIndex;
			threads[threadIndex] = std::thread([ticketNumber]() {
				executeTicketingSystemParticipation(ticketNumber);
			});
		}
